...
Ask / consult commands:
  ...
  prodex pro browser check [--source-cli /absolute/path/to/dist/cli.js] [--cwd /absolute/path/to/repo] [--port 9333] [--timeout-ms 15000] [--runtime]
...
export function printProBrowserHelp(stdout: (line: string) => void, sourceCli?: string): void {
  ...
  const checkUsage = sourceCli
    ? `${cli} pro browser check${sourceCliOption} [--cwd /absolute/path/to/repo] [--port 9333] [--timeout-ms 15000] [--runtime]`
    : "prodex pro browser check [--source-cli /absolute/path/to/dist/cli.js] [--cwd /absolute/path/to/repo] [--port 9333] [--timeout-ms 15000] [--runtime]";
...
check verifies configured selectors under the shared browser lock without sending or navigating. It opens the model menu, inspects effort steps and verifies restoration; busy, missing or unverified selectors exit nonzero. With no selector defaults it reports scope=session-only. Base status defaults to 1500ms and selector inspection to 15000ms; --timeout-ms overrides both, with bounded cleanup after inspection. UI readiness is not proof of a Pro answer.
