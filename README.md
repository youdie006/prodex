...
`release pack` does not publish anything. It still refuses missing publish metadata, non-regular or hard-linked packed files, and missing package release checks; it only normalizes packed file modes in the staging copy so package `bin` entries remain executable and other packed files become regular `0644` files. Run `npm run release:verify` and `gptprouse release status` before publishing the tarball it creates. When the tarball is ready, `release pack` prints `release_pack_git` and `release_pack_git_next` lines before the publish commands so git remote/upstream blockers stay visible, then prints both `npm publish --dry-run <tarball>` and `npm publish <tarball>` commands for that exact tarball path.
