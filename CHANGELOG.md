...
### Fixed
- Composer tools now recognize the measured `Add files and more` button when ChatGPT omits the old plus-button test ID. Lookup stays within one visible composer, rejects ambiguous/hidden/disabled controls, and strictly verifies the trigger's hit target before initial or retry clicks. The updated WSL host installation passed a live no-send Web search activation and restoration check. The [tools incident record](docs/composer-tools-incident-2026-09-26.md) also separates a reproduced stale-container transcript/captcha false positive from genuine human-verification requirements; the container itself has not been replaced.
- Current ChatGPT model menus are recognized through their trigger's exact ARIA relationship when the old picker test ID is absent, and keyboard input goes to the visible reasoning control rather than its hidden slider thumb. Current sidebar project rows use exact names and hover-revealed new-chat buttons with freshly resolved, strictly hit-tested coordinates. `pro browser check` now checks configured model/effort/project controls under the shared send lock instead of declaring readiness from login and composer presence alone; missing, busy, incomplete and unrestored checks fail closed. See the [selector incident record](docs/selector-incident-2026-09-26.md) for live evidence and deployment limits.
- ChatGPT's measured fallback transcript layout now contributes correctly paired user/assistant turns instead of looking empty or treating words such as "captcha" inside a question as a live blocker. Post-submit acknowledgment uses the same reader so a recognized posted question does not trigger extra send-button attempts. Ambiguous mixed layouts, duplicate coordinates and conflicting roles fail closed; an accessibility role heading is excluded only for the observed assistant-body structure. Missing rendered model metadata remains unknown, never verified Pro. See the [source and read-only verification record](docs/fallback-transcript-2026-09-26.md); existing services have not been replaced.
- Headless login now checks the actual dedicated browser process before recording or describing a headless launch and again before reporting readiness. A missing or stale launch record can no longer turn reuse of a headed browser into a false headless success; unverifiable or mismatched processes stop without closing the browser or opening another window. See the [verification record](docs/headless-mode-truth-2026-09-26.md); this corrects mode reporting, not the separately measured ChatGPT protection refusal.
...
### Changed
- Claude troubleshooting now distinguishes the configured stdio transport from the optional HTTP server, so an unrelated HTTP-port owner is not mistaken for the cause of a stdio failure. The HTTP tool list explicitly excludes browser consult/recovery tools, and persisted `<project>` placeholders are documented as privacy redaction rather than evidence of a broken argument. The [selector incident follow-up](docs/selector-incident-2026-09-26.md#reporter-path-and-mcp-follow-up) records the actual reporting directory, fresh MCP handshake and remaining client/deployment limits.
...
### Added
- A [Pro-assisted Ozone experiment](docs/ozone-headless-trial-2026-09-26.md) demonstrates stock Chrome rendering without Xvfb or the browser `--headless` switch on WSL x64 and M3 Linux ARM64. It preserves actual process/security, screenshot and restart evidence, plus an intermittent M3 keyboard failure instead of calling a passing repeat a fix. No candidate public request, login, production migration or protection bypass was performed; the backend remains experimental.
- A [same-build public mode comparison](docs/headless-paired-control-2026-09-26.md) fills the missing Chrome 154 headed controls on WSL x64 and M3 Linux ARM64: both headed documents load with HTTP 200, while each subsequent bounded pure-headless check receives a correlated 307 -> 403 challenge. Local browser operations and cleanup pass; the failure is recorded without claiming a headless fix, retrying protection, prompting for login or replacing the working services.
...
- A [bounded stock Chrome 154 trial](docs/stock-chrome-154-trial-2026-09-23.md) compares pure headless and Xvfb on WSL x64 and M3 Linux ARM64 with pinned artifacts and fresh isolated profiles. Local compatibility and sandbox checks pass, but one public-root headless observation per target returns a correlated 307 -> 403 protection response. Both failures and successful cleanup are recorded without retries, re-login, promotion or changes to the working services.
- A [parallel pure-headless investigation track](docs/headless-parallel-track-2026-09-23.md) preserves the verified WSL/M3 virtual-display service while defining isolated comparison and promotion gates. A bounded upstream-code recheck and account-free WSL headless smoke passed, and both installed sessions remain READY. This records the user's direction; it does not change defaults, retry blocked ChatGPT access or claim a pure-headless Pro fix.
- `prodex login` provides guided authentication for an existing local ProDex container: it discovers the Docker/Colima service, reuses ready sessions without opening a window, and otherwise connects a temporary authenticated viewer without manual Docker commands, port selection or viewer-password copying. The user still handles ChatGPT login/security prompts. `--check` is read-only; ambiguous/remote targets fail closed, and cancellation never stops the saved browser. This is source-only until released, not an automatic container installer or MCP reconnection.
- A [guided-login and M3 verification record](docs/guided-login-2026-09-18.md) records two actual M3 `gpt-6-pro` answers, exact-thread continuation through separate MCP clients, no-send recovery, and trusted persisted receipts/artifacts. It supersedes the older M3 first-login limitation below, but does not pass pure-headless access or change either installed service.
- A [September 18 live recheck](docs/container-browser.md#live-recheck-2026-09-18) confirms two new `gpt-6-pro` responses and exact-thread memory through independent MCP connections on the existing WSL virtual-display service, plus request-specific no-send recovery and trusted artifact verification with no viewer connected. This closes only that bounded operating-mode check: pure-headless access is still blocked, M3 still requires its own first login, and no service, client attachment, package or release was replaced.
- Source-checkout browser smoke now includes [navigation-correlated diagnostics](docs/navigation-diagnostics-2026-09-18.md), with local redirect/child-frame/refusal fixtures and an explicit one-shot `--public-chatgpt` check. Partial failures retain redacted response evidence; public refusals exit nonzero without retrying or opening login. WSL/M3 pure-headless local fixtures pass, but the bounded WSL public check still records a confirmed 307 -> 403 challenge, not a Pro-access fix or installed-service update.
- A deeper [403 measurement and success-claim audit](docs/headless-403-research-2026-09-17.md#deeper-measurement-audit) reproduces the experiment observer's URL-only attribution gap offline, traces the tested Chrome version to V8 fixes that qualify an older `Runtime.enable` explanation, and separates real headless launch code from verified Pro output and exact-thread continuation. Existing browser regressions pass, but this research does not fix remote 403 or change installed services.
- A [GitHub/Reddit 403 research follow-up](docs/headless-403-research-2026-09-17.md) distinguishes actual document refusal from local challenge false positives, records an upstream retracted IP-only diagnosis and its later virtual-display migration, and identifies missing comparison diagnostics. It does not claim a headless-access fix or request another login; installed browsers and authentication are unchanged.
- A [stock Chrome 153 comparison](docs/stock-chrome-comparison-2026-09-17.md) verifies Linux x64/ARM64 vendor artifact integrity and four isolated headless/virtual-display capability checks. Both headless public probes returned HTTP 403 with independent protection-page evidence; the two virtual public probes timed out and remain inconclusive. Existing authenticated services were preserved, with no login or prompt attempts and no browser-default or installed-runtime change.
- A [measured headless/virtual-display comparison](docs/headless-mode-comparison-2026-09-17.md) records 12 isolated WSL/M3 smoke passes with raw timing, CPU and memory observations. It also corrects a prior experiment's argument mismatch by using the actual native headless builder: the same-profile ChatGPT trial still stopped at Cloudflare with zero prompts, and restoration retained the WSL login. The small synthetic resource measurements do not claim production savings or a headless Pro-access fix; runtime defaults are unchanged.
- A source-checkout container client provides `status`, `pro`, `mcp`, and `viewer` commands for an explicitly selected existing Docker context/container. It preserves clean MCP stdio, exact-task continuation, a private bridge directory, and independent client cleanup without automatically starting browsers or logging in. The viewer command derives loopback-only URLs, requires an explicit forwarded port for remote endpoints, and offers terminal-only password copying through private pipes instead of printing the password. Host repositories are not mounted; this is not a new npm command.
- The [container-client installation record](docs/container-browser.md#container-client-installation-2026-09-17) tracks the updated WSL/M3 images, both machines' Codex/Claude configuration and live MCP connections, and one actual fresh-Codex `gpt-6-pro` continuation with trusted request/artifact verification. A same-profile pure-headless trial stopped at Cloudflare with zero prompts; restoring virtual-display mode retained the WSL login. M3 still needs its own first login, and the existing Codex attachment was not restarted.
- An [installed-container acceptance record](docs/container-browser.md#installed-container-acceptance-2026-09-17) verifies the corrected WSL runtime after an authorized container replacement with the same login volume and viewer password: no re-login, two completed `gpt-6-pro` answers, exact same-thread memory across separate MCP processes, and trusted receipts/artifact hashes. All six native CI jobs passed for the installed implementation. This closes the bounded WSL virtual-display Pro gate, not pure-headless access, M3 authenticated acceptance, client reconnection, or public release.
- A [container authentication and live-request record](docs/container-browser.md#authenticated-container-check-2026-09-17) confirms the user's WSL container login and retained readiness with no viewer connected, but records a failed bounded Pro check: one request timed out after 300 seconds and one exact-request, no-send recovery found no completed answer within another 120 seconds. No continuation was sent, no Pro response is claimed, and the manual Docker/password onboarding remains an open usability gap.
- A [live Pro acceptance record](docs/pro-acceptance-2026-09-16.md) verifies three actual `gpt-6-pro` answers through fresh 0.40.18 MCP connections on the existing signed-in WSL host, including new-chat selection, cross-process continuation, request recovery, and unrelated-session rejection. It rejects the attached 0.40.16 runtime's non-Pro answer and keeps client reconnection, container authentication, and pure-headless access explicitly unpassed; no runtime or authentication state was replaced.
- An [isolated true-headless verification record](docs/container-browser.md#isolated-true-headless-baseline-2026-09-16) separates passing WSL x64/M3 ARM64 Linux browser mechanics, actual synthetic pixels, and sandbox checks from blocked ChatGPT access: one fresh-profile navigation per target returned HTTP 403. Trials used disposable containers without existing profiles, attempted no login or prompt, and left the virtual-display services unchanged; this is not a headless Pro-access fix.
- A [true-headless candidate audit](docs/headless-candidates-audit.md) distinguishes Camoufox's actual launch modes and Windows ChatGPT report from verified Pro access, identifies upstream model-selection and continuation risks, and confirms that pro-gate's WSL setup is virtual-display Chrome. It records ProDex adapter requirements without changing browser/authentication state or claiming a headless fix.
- An opt-in [single-user container browser experiment](docs/container-browser.md) keeps Chromium, a private Linux virtual display, and stdio MCP in one runtime on WSL x64 and M3 ARM64. A localhost-only, password-protected viewer exposes that same browser for manual login without a host desktop window or a headed-to-headless handoff. A dedicated volume preserves the profile and viewer password; a stable hostname avoids false cross-computer profile locks after container replacement. Startup never sends a ChatGPT prompt and failures stop instead of restarting in a loop. This is virtual-display Chromium, not verified authenticated headless Pro access or a default installation change.
- The disposable browser smoke accepts explicit `--headed` verification while retaining headless as its default and rejecting unknown options before launch. The container MCP smoke checks two independent processes, synthetic task identity, exclusive claims, and client-disconnect cleanup without sending browser prompts.
- An account-free container viewer probe verifies Chromium's sandbox diagnostics, authenticated VNC pixels, viewer disconnection without browser replacement, and synthetic-state persistence across an external container restart. Its verification mode cannot seed the expected marker, and real existing pages block mutation.
...
### Fixed
- Browser compatibility evidence declarations now accept headed observations and distinguish `headed_process` from `headless_process`, matching actual smoke results instead of typing every successful run as headless. An offline compiler regression covers both modes and rejects contradictory output markers; browser behavior is unchanged.
- Container-viewer guidance now distinguishes the browser's computer from the agent/terminal host before selecting localhost ports or clipboard commands. A [recorded M3 correction](docs/container-browser.md#m3-local-viewer-correction-2026-09-18) replaces a wrongly supplied WSL-only login route with the existing direct M3 viewer and a user-triggered native clipboard prompt; no browser profile, password, runtime or protection policy changed, and authentication is not claimed as passed.
- Authentication-control recognition also preserves exact `Sign up for free` and Korean free-signup labels inside navigation, including when an anonymous composer exists. Excluding sidebar protection text no longer drops these already-supported login signals.
- Browser status and answer checks exclude navigation/history control labels from protection detection as well as page text. A sidebar conversation titled `Just a moment...` no longer falsely triggers `cloudflare_check`; real non-navigation protection, login and limit evidence remains blocking.
- The account-free browser restart smoke follows the exact navigated CDP target ID and expected URL. A restored tab at the same fixture URL no longer causes a false failure or substitutes for a missing/wrong-page target; genuinely ambiguous unpinned lookups still fail.
- The account-free container viewer smoke waits for the actual fixture/viewer document URL as well as its completed load state. Previously, a new target's still-complete `about:blank` document could pass readiness and fail noVNC's relative module import before the viewer loaded.
- The experimental container runs the packaged Chromium binary directly. Debian's `/usr/bin/chromium` wrapper was adding `--disable-dev-shm-usage` under its 3.8 GB threshold, silently redirecting the configured 1 GB shared-memory workload into the 256 MB `/tmp` mount; it also forced GPU rasterization. The container now retains its own launch settings and explicit background-network/extension restrictions and build-checks the binary path, without changing browser version, sandbox, authentication, or host-browser defaults. WSL x64 and M3 ARM64 candidate images pass account-free viewer/MCP checks and use `/dev/shm`; the later WSL installation also passed a bounded Pro answer and continuation without reproducing SIGILL. This does not prove a universal crash fix.
- Container viewer guidance now distinguishes the web browser's localhost from an SSH terminal's host, requires retrieving the password from the matching Docker context, and gives a loopback-only SSH forwarding recipe with explicit noVNC endpoint settings. This addresses cross-host password failures observed when the M3 viewer was opened with the WSL viewer's password; it does not reset authentication or claim headless Pro access.
- Container-related checkout files and the Linux-executed `prodex.mjs` launcher retain LF endings on Windows, preventing a CRLF shebang from breaking the in-container command. Build-context tests accept either source line ending; runtime images already tested on WSL/M3 are unchanged.
- Browser login waits no longer open another ChatGPT tab on every poll while an existing tab redirects through authentication. A wait can attempt one initial tab opening, reports a failed opening without retrying it, and preserves an already-observed login flow. Existing Google/OpenAI sign-in pages are reported as pending manual authentication instead of a missing ChatGPT tab; their contents and URL parameters are not read.
- WSL source-checkout startup guidance now covers Linux-backed dependencies when a Windows-mounted `node_modules` makes CLI version checks and stdio MCP initialization time out. The dependency ignore rule also covers symlinks so the local cache path cannot be accidentally staged. The measured local repair retains the source directory and lockfile; it does not change browser authentication or production deadlines.
- The explicit-directory HTTP MCP startup test waits for the server's readiness signal before requesting shutdown, instead of emitting a signal after an assumed 50ms startup. This prevents a missed shutdown and false timeout under filesystem load while retaining the existing working-directory and token-redaction assertions.
...
### Fixed
- The observed Cloudflare 502 page is classified as `chatgpt_service_error`, not a lost login. Detection uses message-excluded page text and preserves genuine authentication/protection blockers; normal conversations discussing the error remain unaffected. An error after submission retains the non-retryable, no-automatic-resend guard. This reports the upstream failure; it does not repair the ChatGPT service or bypass Cloudflare.
- Browser product checks now report `browser_process_unverified` and finish the remaining diagnostics when OS process identity cannot be inspected. They leave the existing browser unchanged instead of crashing or recommending login/reset. Connection-refusal fixtures no longer depend on unrelated browser processes on the test host; strict ownership checks before shutdown remain unchanged.
...
- Native Windows repository search discovers `rg.exe` and returns the same forward-slash repo-relative paths as other platforms. Windows alternate data streams are rejected by repo path policy; POSIX colon-containing file names remain supported. File-link security tests continue to fail when the host cannot create their fixtures instead of silently passing them.
- Native Windows npm operations invoke npm's JavaScript entrypoint through the current Node runtime instead of executing `npm.cmd` directly. Installation/package checks retain literal arguments, timeouts and credential-isolated dry runs. Generated Windows commands use PowerShell quoting; POSIX quoting remains unchanged.
- Browser process verification uses structured Windows CIM records as well as POSIX process listings. Handoff verifies the dedicated browser's executable, main PID, port and profile before shutdown; incomplete or ambiguous identity remains a blocker. Browser launch smoke uses a disposable, account-free headless profile and checks real CDP evaluation and owned-process cleanup.
- Concurrent Windows bridge initialization accepts a replacement conflict only after reading the exact completed gitignore from the competing initializer; missing, different, unsafe and permission-denied results still fail. The unchanged-file path checks that the ignore file still exists. POSIX artifact names containing colons can again be removed through the anchored writer; Windows alternate-stream syntax stays rejected.
- Native Windows file replacement closes its content-verification handle before atomic rename, retaining preimage checks and leaving late-created hard links unchanged. Repeated initialization stops rewriting an already complete bridge gitignore, avoiding concurrent Windows replacement failures. Anchored-writer fault injection now exercises rollback and directory-swap protection on macOS/Windows as well as Linux.
- Native Windows bridge initialization no longer fails with `EPERM` from POSIX directory-handle permission changes. Directory identity and junction rejection stay enforced; Windows storage uses inherited ACLs rather than claiming Unix modes make it private. The anchored writer rejects Windows separators, drive-relative paths and alternate-stream syntax.
...
### Changed
- Independent project-warning and terminal-state cases now run as separate tests with unchanged assertions. The eight-task concurrent persistence check has a dedicated 60-second budget for native child-process-backed storage; the global timeout and production limits are unchanged.
...
- CI runs full release verification on Ubuntu 24.04 with Node 20/22/24, macOS 15 ARM and Intel with Node 22, and Windows Server 2025 with Node 22. Jobs install/check ripgrep, report their actual runtime, fail independently, and have a 40-minute deadline. Passing this matrix does not establish support for every historical OS, architecture or browser version.

### Verification And Limits
- PASS at `ba9551be75586b7ab603964a6cbf117483578766`: all six native CI jobs completed full release verification and account-free headless browser smoke. Ubuntu Node 20/22/24 each passed 1,614 tests with 3 platform exclusions; macOS ARM/Intel each passed 1,613 with 4 exclusions; Windows passed 1,615 with 4 exclusions. Every job had zero failures. See [CI evidence](https://github.com/youdie006/prodex/actions/runs/34980308069).
- PASS: full release verification on local WSL (1,614 tests) and physical M3 (1,613 tests), including typecheck, build, installed-package CLI/HTTP MCP/stdio MCP smoke and doctor. Native Windows also verified an actual installed `prodex.cmd` in a path containing spaces, a fresh non-tmux MCP with 20 tools, and explicit working-directory file reading.
- Earlier runs failed on native permissions, atomic replacement, package modes, npm environment casing, host process inspection and overloaded test cases. Focused regressions failed before correction, followed by the passing matrix above. The local Windows 11 host still lacks file-symlink fixture privileges; those local cases are not claimed as passed. Full Windows security coverage ran on the CI host. Detailed attempts and prerequisites are retained in [Platform Verification](docs/platform-verification.md).
- Before OS verification, saved-login WSL virtual-display Chrome returned a verified Pro answer and the corrected local continuation returned `43` from the prior answer's `42`, with exact request/thread and artifact checks. Virtual display is headed Chrome without a desktop window, not pure headless access. M3's earlier pure-headless Cloudflare blocker remains unresolved. The later OS and 502 tests used disposable or simulated data, not new account requests; no visible fallback or protection bypass occurred.
- The version-specific GitHub Release records the final tag/commit, npm publication and per-machine installation/runtime checks. A source or package update does not reconnect the existing MCP. No Codex/MCP restart was performed; service-side 502 errors are reported, not repaired by this release.
...
- A temporary visible authentication recovery no longer replaces the user's headless relaunch preference. Current-process mode remains accurate, reusing an open authentication window preserves the preference, and the next launch after that window closes is headless unless the user explicitly selects another mode. Saved launch metadata is replaced atomically with private file permissions so concurrent readers do not observe a partially written mode/profile record.
- Automatic browser restart now holds the same cross-process send lock as consultation and rechecks readiness after acquiring it. It reuses a browser that recovered while queued, refuses reachable authentication/protection or slow states without restarting, and keeps the saved-profile/control-port guard before any READY reuse. One session's recovery can no longer race another cooperating session's active send through this unlocked path.
- HTTP errors, malformed control replies and uncertain connection failures no longer count as a stopped browser or recommend another login/reset. Automatic restart requires refused-connection evidence; queued READY reuse verifies that the saved profile owns the actual dedicated process and rejects missing or mismatched identity, including partial records with another port.

### Verification And Limits
- Final local release components passed: the full 1,541-test suite, typecheck and build through `release:verify`, followed by the corrected `npm run smoke:package` and `node dist/cli.js doctor`. Node 20 focused regressions passed (92 tests); CLI/MCP send regressions passed separately (120 tests). Earlier runs exposed obsolete help/login expectations, four fixtures missing the pre-launch check, six fixtures missing refused-connection evidence, and one remaining installed-package login expectation for an HTTP 503 error. Those fixtures/assertions were corrected and the affected checks rerun. New preference, atomic-record, profile/port identity, control-error and misleading-reset regressions failed before their fixes and passed afterwards.
- Isolated WSL and macOS Chrome tests with fresh temporary profiles and only `about:blank` verified that a saved temporary-visible record launches an actual `--headless=new` process without a desktop window. Both test browsers exited and their temporary profiles were removed. This is launch-mode verification, not a logged-in ChatGPT or Pro test.
- Both deployment targets installed the final candidate and fresh non-tmux stdio MCP processes reported 0.40.15 with 20 tools including `pro_consult`; runtime hashes matched the local build. The headless relaunch preference was restored under the shared send lock without closing/opening the real browsers or touching their authentication data. Existing client-managed MCP processes were left intact.
...
- Source/candidate verification is separate from public publication and installation. The version-specific GitHub Release records the final tag/commit, npm and GitHub workflow results, installed versions, and fresh-process verification on each deployment target. Existing client-managed MCP processes need reconnection to load an installed update; package installation alone does not replace them or restart Chrome.
...
- Tag and manual publishing run the same release metadata check as CI before publishing with lifecycle scripts disabled.

### Changed
- Refresh vulnerable production and development dependencies, including the Vitest security minimum. Document non-tmux operation, foreground process lifetime, and independently expiring ChatGPT logins.
...
### Fixed
- A consult that had to restart the browser said nothing about it. Auto-recovery records what it did so the receipt can carry it, but only on the branch that ENDS a browser that stopped answering - and the commoner case, where the browser is simply gone and prodex starts one, pushed nothing. Measured by killing the dedicated browser and sending with `--auto-login`: recovered in three seconds, answered normally, and left `warnings: []` on the receipt, the result and the task, so nothing could say the answer came from a browser prodex had to start. Both halves are recorded now. The notes also survive a retry that then fails - they used to be attached to the answer, so a recovered browser whose retry died recorded neither the recovery nor the reason.
...
### Fixed
- `--tool create-image` worked once per account and failed forever after. The lookup searches the document for the tool's name, and the first successful send leaves a conversation titled "Create Image" in the sidebar - so every send after it matched that chat row, clicked it, navigated to the old conversation, and then reported that the composer never showed the tool as active. The sidebar, the navigation and the message list are excluded from that search now. (Scoping it to an open menu was tried first and broke every tool, web-search included: this build's menu carries no menu role, no aria-controls and no floating popover node, so nothing matched it.)
- A send that started right after another one could die on "model selector button not found". The composer is a moment behind a page that has just navigated, and the poll allowed four seconds while the neighbouring waits for the same kind of render allow six to eight. Measured on the pattern this shared browser exists for - two agents sending one after the other, the second queued on the lock: it failed once in three attempts and succeeded either side of it. The budget matches its neighbours now, and the failure is named as the transient it is, asking for a retry instead of sending the caller to investigate a browser that is fine.
- `prodex start` reported a port clash as `listen EADDRINUSE: address already in use 127.0.0.1:8787` and nothing else. Measured here: the default port was held by an unrelated program, and the message named neither who had it nor the one flag that moves prodex off it. It says both now.
- `pro blockers` grouped old records by the code they were written with, and most recurring failures were recorded as the catch-all before they had names - so the report kept showing `browser_send_failed: ...` rows for causes the classifier now knows. Catch-all records are re-read through the current classifier when the report is built; a record that already had a code is left exactly as written, so a later classifier cannot change what an old failure was. The 30-day picture on this machine went from rows of quoted messages to `selection_not_applied` 14, `composer_not_ready` 7, `click_blocked` 5 and the rest by name.
- `pro show` printed a failed send's budget and not its request. With the selection now recorded on failures, it prints `asked_for: model=... effort=... project=...` beside the blocker, so a timeout can be read against what it was for.
- `--continue` could never find a project written in another script. It matched threads by the project's name as it appears in the URL, and four of this account's five projects have Korean names: the slug prodex derived is "" for two of them, and ChatGPT romanizes the name in the URL anyway (a project called `prodex-probe-한글` answers on `.../g-p-<id>-prodex-probe-hangeul/c/...`), so no follow-up into such a project would ever have matched, first consult or hundredth. The project's id is read off the live sidebar now - exact name first, then a case-insensitive match when it is the only one - and threads are matched by that id, which every thread URL carries. Verified live: a Korean-named project created, sent into, and continued by name in the same conversation.
- prodex's own test runs filled the user's bridge registry with throwaway directories. The registry is what `pro blockers` scans, and it read "across 77 bridge roots" on a machine with 19 real ones: 54 were `/tmp/prodex-mcp-target-*` from one vitest test that spawns `prodex mcp` as a child process - the MCP SDK hands a child only a short allowlist of variables unless told otherwise, so the isolation the test setup puts in the environment never reached it, and every `npm test` registered one more. `doctor` added a dead entry per run the same way, through the temp bridges its smoke checks build and delete, and `release:verify` added nine per run through the package smoke. Bridges prodex builds for itself can now say they are throwaway (`registerRoot: false`), the doctor smokes do, the vitest test asserts its root landed in the isolated registry and nowhere else, and the package smoke points every child at its own registry and login record. The browser send lock is deliberately left shared: the smoke sends through the real browser, and that lock is what keeps it from typing into another session's consult.
- A send that failed did not record what it had asked for. All 57 `send_timeout` blockers in this machine's ledger carry no selection at all, because the model, effort and project were only ever written into the ANSWER receipt - and a timeout never writes one. So the ledger cannot answer the first question anyone asks of a timeout: was this a Pro run, and was the budget right for it. Failed sends now record the selection alongside the blocker, with the project name scrubbed the way the rest of that record already is.
- Most of what actually fails was reported as "resolve the visible browser issue manually". Auditing the ledger on this machine - 296 recorded blockers across 77 bridge roots - showed 178 of them in the catch-all, and replaying their messages through the classifier showed 109 still landing there. The recurring, still-reachable ones now carry their own code and their own next step: a control something was covering (`click_blocked`), a composer that did not end up holding the prompt (`composer_text_mismatch`), a picker that had not rendered (`composer_not_ready`), a tool that never turned on (`tool_not_applied`) or is not offered at all (`tool_not_offered`), a project name the sidebar does not have (`project_not_found`), a sidebar click that did not move the tab (`project_navigation_failed`), and an attachment still uploading when the budget ran out (`attachment_upload_timeout`). Each says that nothing was sent, which is a fact on all of these - they all fail before the prompt is submitted. Replayed against the same ledger, 141 of the 178 now classify; the rest are failures whose code paths no longer exist.
- A send that failed dropped everything it had already noticed. Warnings ride out with the RESULT, so a throw discarded them - including the ones that would explain the failure, like having just moved the browser off ChatGPT's Work surface or having recovered it, collected moments before the step that died. Every caller saw `warnings: []` on a blocked send. They now travel out with the error the way the thread already does, and reach stderr, the `--json` output and the blocked record.
- An answer whose recording failed reached a person but not an agent. The send prints `consult_answer_received_but_not_saved`, the thread and the answer before throwing, so a terminal still has the text; the MCP path rethrew and discarded the collected output - losing exactly the answer that cost the most to get, a Pro run someone waited minutes for, because a receipt could not be written. `pro_consult` now returns it with `status: "answered_not_saved"` and the failure in the notes.
- `--allow-model-fallback` promised something it does not do. Its description said a send whose model could not be applied stops unless the flag is passed; measured, `--model "GPT-4 Turbo"` without the flag sends anyway with a warning. That is the right behaviour and it is deliberate - this picker's model rows carry `pointer-events: none`, so refusing would block every `--model` send on every account - but the flag gates the EFFORT step only, and the description now says so, and points at `model_used` and `pro_verified` for what actually replied.
- `--file` inlined a binary's bytes into the prompt. It puts a file's TEXT into the prompt while `--attach` uploads the file itself, and pointing it at an executable produced a preview carrying an ELF header and NUL bytes inside a text fence - 535 control bytes out of 798 - which costs tokens, answers nothing, and pushes control characters through the composer and the prompt-identity check. It now refuses and points at `--attach`.
- `doctor` reported `config: ok` for an endpoint that could never be served. Measured here: the configured port was held by an unrelated program, so `prodex start` failed with EADDRINUSE every time while the one line someone reads before believing the setup said it was fine. It now checks whether the port answers as a prodex server, and says so when a stranger holds it.
- A follow-up could pick the provisional conversation id ChatGPT shows between the prompt posting and the server naming the thread (measured as `/c/WEB:<uuid>`). The id parser used for navigation already refused it, so the two disagreed: `--continue` would accept such a record and then fail to open it with a complaint about the URL rather than about the conversation.
...
- A conversation that cannot be opened arrived wearing the catch-all. Measured on a thread whose project had been deleted: the message said "It may have been deleted" and the next step underneath said "resolve the visible browser issue manually" - about a browser that was working, with a retry suggestion for something retrying cannot fix. It has its own blocker now, not retryable, naming the two ways forward.
- The composer of a plain new chat read as a label prodex could not parse. Measured on a live root: the hidden fallback textarea says "Ask ChatGPT" while the editor prodex actually reads says "Chat with ChatGPT", and only the first was known - so the clearest case of "this composer is not the project's" was reported as a label that could not be read.
- A turn that finished without writing text was waited on forever. Measured with `--tool create-image`: the image was generated and rendered, its assistant message came back from the transcript finished with an empty part (the image lives in a separate tool message), and prodex spent six and a half minutes "stabilizing" before reporting a timeout for a result already sitting in the thread - which `recover` then could not read either. The transcript only reports an empty answer AFTER checking the turn ended, so it never meant "still being written". Such a turn now comes back promptly, saying the answer is not text and pointing at the thread.
- The corrupt-config refusal offered a way out that does not exist. It told the caller to pass the defaults explicitly instead, and the config is read before any flag is looked at, so that hits the same error - advice that does not work is worse than none. It names the one that does: move the file aside if the repo does not need the HTTP MCP surface.
- Callers were never told where the answer landed or which conversation it continued. Both were in the receipt and in a progress line, and MCP notes drop progress lines, so an agent asking for a follow-up could not tell which thread it got - or whether the answer is in the project it asked for. `destination` and `continued_from` now come back in the `--json` output and in the notes beside `model_used`.

## 0.40.3

### Added
- `--continue` (and `continue_thread` on `pro_consult`) sends a follow-up into the conversation a previous consult is already in, resolved from this repo's own `.bridge` records - the newest FINISHED consult of the same project - rather than from whatever the shared browser tab is showing. `--continue-task <task_id>` names one exactly. It refuses instead of guessing when the project has no finished consult yet, and the receipt records `continued_from`, so a chain of follow-ups is auditable.

### Fixed
- The consult tool told agents that follow-ups stay in one conversation by default. On any machine with a project pinned they never did: entering a project navigates away from the open thread and starts a new chat there, so every consult was a fresh thread. Measured - two consecutive sends into one project landed in two different `/c/` threads, and the follow-up answered correctly only because ChatGPT's project memory carried it, which is why nobody noticed. The description now says what actually happens, and `--continue` is the way to follow up on purpose.
- **0.40.2 refused every send into a project.** The binding check reads the label the composer names its project with, and it read only `data-placeholder` - which is null on the editor prodex actually types into. Measured on a live project home: the contenteditable div carries the label as `aria-label`, and the only element with a `placeholder` attribute is the hidden 0x0 textarea the composer finder rejects on size, correctly. So the label was on the page, prodex could not see it, and the fail-closed branch added in 0.40.2 did exactly what it was built to do on a page that was fine. The label is read from either attribute now. Verified live end to end: a `--project-new` send and a cross-project `--project` send both bind and post into the right project.
- A logged-in Pro account on a working page was told to go and log in. The login check reads the page text for the sidebar furniture - "New chat", "Projects", the plan hint - and it read a filtered sample that keeps only text whose own parent element has a box. Measured live on a project home: that sample was 111 characters of a promotional banner while `document.body.innerText` carried the whole sidebar, so a send onto a page with a working composer refused with "missing a clear logged-in ChatGPT session. Log in manually". Reproduced twice in a row; four of the last thirty days of recorded blockers carry that message. The two halves of the verdict now read the text each needs: whether the app is here is answered from either sample, while whether this is a login screen keeps the message-excluded one, so an old chat quoting a signup page still cannot fake a logout.
- A send into the project the tab was ALREADY in still clicked the sidebar row, which can only fail: measured, the click refused with "another element covers its click point" on a page that was ready to accept the prompt. The composer is asked first, and a composer that already belongs to the requested project needs no navigation.
- Receipts record where the answer actually IS, not only where the send aimed. The project id the composer was confirmed bound to before typing is compared against the project the answered thread belongs to, and the result is recorded as `destination: { observed, verified }` - with `verified: false` for a landing nothing could check, because a receipt that certifies what nobody checked is how a misdelivery goes unnoticed. The comparison uses the project id rather than the URL slug: measured within one send, the same project renders as both `/g/g-p-<id>/project` and `/g/g-p-<id>-<name>/c/<id>`, and comparing those whole would report two different projects. A thread that lands in another project, or outside every project, says so and leaves the other project's name out of the record.
...
- A tab that stopped answering was reported as a project that would not bind. The poll that reads the composer's placeholder swallowed every failure, including the command timeout that means the browser is gone, so a dead tab spent the project budget and the recovery's before refusing with "the composer's placeholder could not be read" - the one diagnosis that sends the caller looking at ChatGPT rather than at the browser.
- A send that created its project kept that project's real name in the record it wrote. Blocker text quotes the project, and the persisted task and session are scrubbed because they cross the MCP boundary, but only the REQUESTED name was ever scrubbed - and a `--project-new` send has no requested name. Both are scrubbed now, longest first, so a name that contains another cannot leave the rest behind.
- `prodex pro blockers` printed a cause's oldest wording next to its newest date. The example was whichever record was read first, while the timestamp tracked the newest, so a cause holding a fixed failure and a current one showed the fixed one as what is failing now - measured here: a picker failure from before the 0.39 fix was displayed with a date from a week after it.
- The fallback that finds a project row when its options button cannot be read still matched by substring, so asking for "Codex" took "Codex Review" - the exact wrong-project send the equality check on the primary path exists to prevent. It compares the row's name now and refuses when two rows carry it.
- Refusing to post into a project prodex cannot confirm was a plain error whose English prose a regex recovered the code from, so a reworded message would have quietly become an unclassified send failure. It carries its own blocker, with the reason kept apart from the sentence; the wording still holds the phrase the text classifier keys on, for the paths that reach it as text.
- A browser send lock that could not be removed spun the waiting process with no sleep and no deadline - a hot loop that neither returned nor timed out. It waits like any other contended lock now, and says which file to delete when the budget runs out.
...
### Fixed
- The power slider was looked for in the whole document while only its PRESENCE was checked inside the picker, so any other slider on the page became the one prodex focused and sent arrow keys to - in a real logged-in session. Both now look inside the picker, with a document-wide fallback only where there is no picker to look in.
- Narrowing the model listing to radios emptied it on the picker that keeps models behind submenu rows, which are menuitems. Radios and submenu rows are both listed; the power slider's own rows - its track and the pair naming the live model and effort - are what gets left out.
- Reading which surface is live went through the click-point helper, which scrolls the element into view and writes an attribute. That ran on every send, and up to fourteen more times while confirming a switch, scrolling the page to answer a question. The read is now a read.
- Switching surfaces only worked where the toggle is drawn, which is the home screen alone. A send into a project or onto a thread found no toggle and gave up with a warning, on exactly the pages where the drift matters most.
- `--effort Max` and `--effort Ultra` were advertised and then made impossible: sends put the browser back on Chat, and those two are rungs of Work's slider. Asking for one now keeps the send on Work; everything else goes to Chat, whose top step is Pro.
- A thread could be left rendered with no composer at all - measured after a send, zero contenteditables and zero textareas - and every retry hit the same dead page with the same "missing a visible prompt composer". One reload is spent before reporting it, which recovered the send that had been blocked.
- The usage lines never mentioned Max, Ultra or Pro among the effort values, so nothing in the help led to the steps the parser accepts.
- The composer reload could hit a thread still writing its answer. A streaming thread hides its composer too, so the reload meant for a dead page was reloading a live one - tearing down the answer being written, and doing so before the busy check that should have queued behind it. Gating it on the page's generating flag alone would have refused the reload on exactly the page it was written for: a finished thread keeps its stop button for thirty seconds (measured), and that flag reads it as generating. The reload is now decided after the busy check, from the transcript-checked verdict, and a page waiting for a response choice is never reloaded. It also runs on a connection held open through the navigation - the throwaway connection it used before closed right after asking, which would have left a beforeunload prompt to a client the browser will not let dismiss it - and any dialog answered there is on the receipt.
- Switching surfaces through the persisted value confirmed itself by reading back the value it had just written, so a send could proceed against a page still mid-reload. Polling readyState and the composer instead is not enough either: both are already "complete" and present on the document being left, for as long as the reload takes to commit. The document about to be reloaded is now stamped, and only a document without the stamp - the new one - that has rendered its composer counts as the switch having taken. The project rebind reload had the same hole: it polled with no delay after asking for the reload, and a poll that ran before the reload committed found the old document's composer and URL still matching. Measured live, a reload commits within about 260ms, so the window is narrow rather than absent. It uses the same stamp, and the new document showed its composer 3.2s in.
- A command timeout inside the surface probe - the first command of most sends - was swallowed as "surface unknown", and the next command then reported a closed socket, which named neither the stalled tab nor the dialog cure the timeout guidance carries. The probe now fails the send on a timeout, and a socket closed by a timeout says which timeout when a later command finds it closed. A connection that closed with no timeout behind it - the tab closed or navigated from outside, or the browser gone - is reported as `browser_connection_lost` with its own guidance, where it used to be the generic "resolve the visible browser issue manually".
- `pro browser models` on a thread or project page said nothing about which surface it read, because those pages draw no toggle; the persisted choice, which is what the app itself reads there, now answers.
- `prodex setup` lingered after finishing. The pinned-default check races the picker listing against a 15s deadline, and the deadline timer was never cleared; the CLI exits through the exit code, so the live timer held the process open for the whole budget after the listing had answered. Measured after the fix, a pinned setup returns as soon as the listing does (8.7s against a live browser). When the deadline wins instead, the listing is abandoned at its next step - its browser connection closed as soon as it holds one, and the send lock released with it - rather than left running to completion behind the prompt. The steps before that connection (finding the page, settling its status) are bounded by their own ceilings and finish on their own.
- A send refused by the browser lock was reported as a browser issue to resolve by hand. It is another prodex send holding the window, and the guidance now says so.
- Warnings recommended `prodex setup --model ""` to clear a saved model; that command fails with "--model requires a value". They now name `--clear-model`, which works.
- A stored surface value prodex did not recognise was treated as Work, which would have rewritten the preference and reloaded the page on every send. Only a value naming Work moves the browser.
- A slow browser was mistaken for a dead one, and then ended. The page lookup mapped every failure to "No Chrome DevTools endpoint is reachable" - a timed-out answer included - and the silence check that decides whether to end a browser counted three 2s timeouts on a loaded machine as three signs of death, which its own comment had warned about. What followed was SIGTERM to a live browser mid-answer. A timeout is now reported as `browser_slow` ("running but busy"), only after a raw TCP connect to the port is accepted - measured on WSL2, a closed loopback port does not refuse, it times out, so a timeout alone proves nothing - and only a refused or unanswered port counts as evidence of death.
- Ending someone's browser was narrated only as a progress line. A send that ended and relaunched the dedicated browser now carries `browser_recovered: ... prodex ended it (pid N) and started a fresh one` on the receipt, where `pro latest` and the MCP result show it.
- A JavaScript dialog wedged prodex permanently. A dialog halts the page's main thread, so every `Runtime.evaluate` after it waits forever - measured against the live browser, and `Page.enable` stops answering too, which means a client that had not enabled the Page domain BEFORE the dialog appeared is told "no dialog is showing" when it tries to dismiss one. There is no recovery after the fact, only prevention. It surfaced as `Chrome DevTools command timed out: Runtime.enable`, which names neither the cause nor the cure, and it took a browser restart every time. Every connection now arms dialog handling as its first act: beforeunload is accepted, because that is the page questioning a navigation prodex itself asked for, and anything the page raised on its own is declined rather than agreed to in someone's real session. Verified live - with a dialog open, an armed connection still evaluates.
- A send now says when a dialog was answered on the caller's behalf, rather than pressing a button in someone's session quietly. Verified live: with a beforeunload handler on the page, a send carries `dialog_answered: ... (beforeunload) and prodex accepted the page's leave prompt`, and finishes - where it used to hang for good.
- `Chrome DevTools command timed out` blamed a very long thread and advised a retry. A dialog produces the same timeout and no retry can get past it, so the guidance now names both causes and the cure that works for the second: close the dialog in the visible window, or reopen it.
- `pro browser recover` navigated the tab without holding the browser send lock, so recovering an answer could walk the browser off a thread another consult was streaming into - and recovery is exactly what gets run after a send appears to hang, so the two meeting is a when rather than an if. Its MCP tool description also called it safe to retry because it sends nothing, which skipped over the navigation.
- `model_not_applied` said the picker "does not offer" a model and then listed that same model in the next clause. Measured on both surfaces, the model rows carry pointer-events: none and no coordinate reaches them, so a listed row that refuses the click now reads as exactly that, and the help no longer implies `--model` can pick anything but Pro.
- Reading the model list did not hold the browser send lock, so `pro browser models` could open the picker and walk the slider underneath a consult that was streaming in the same window - two processes driving one browser, with the listing free to change the model the send was about to use. It now waits for a send in flight, and `setup`'s check gives up immediately rather than opening a menu under one.
- `prodex setup` walked the live power slider. Saving a config file connected to the browser and stepped the control from end to end to check what the picker offers, which moved someone's setting and left it moved if the walk was interrupted. The check compares model names, so it no longer walks anything - and it is raced against a deadline, because the timeout it was given covers finding the page and not what happens in the picker afterwards.

### Changed
- The README is rewritten around what the tool does for someone reading it for the first time: a recorded terminal demo (`docs/demo-cli.webp`, rendered by `scripts/make-demo.py` from a real Pro run, timings included), a real console transcript, and sections that follow the questions people ask in order. The release process moved to `docs/releasing.md`. The old demo GIF showed output the CLI never printed and is gone.

### Internal
- Two tests set the DevTools port variable and then deleted it, which sent every test after them in that file back to the real default port - a hole in the isolation that a full run still walked through. They now put the isolation value back. The isolation itself is set unconditionally, so a port exported in a developer's shell cannot route the suite at a real window.
- The test suite drove the real browser. `prodex setup` verifies a pinned default through the DevTools port, and 9333 is where a developer's own logged-in window listens - measured, a full run made five requests to `/json/list` there, each going on to open the picker and, once the listing learned to walk the slider, to step it end to end. It surfaced only as a 30s timeout. Runs now default that port to one nothing listens on; the handful of tests that are about default-port guidance take the variable back for their own duration.

### Added
- The surface can be set where the toggle is not drawn, by writing the choice ChatGPT itself persists (`oai/apps/tpp/chat-surface-mode`, and a host-only `oai-chat-surface-mode` cookie) and reloading onto it. The cookie is read by name rather than by suffix, and written host-only the way the app writes it - a domain-wide copy would sit alongside the app's own, with no telling which one won.
...
- `--effort` accepts `Max` and `Ultra`, the two rungs above Extra High. They appear once a model row is chosen rather than the recommended set: with the set active the slider walks a short mixed ladder that ends at Extra High, and choosing the model swaps in that model's own ladder, which continues past it. Verified live at the top: `thinking_effort: ultra` on `gpt-6-astra-wm`. Those rungs are not in every rollout - a second profile on the same account tops out at 매우 높음 with a step named 6-pro instead - so `max` still means 매우 높음 for anyone already typing it, `Max` and `Ultra` reach the new rungs by name, and `Pro` matches 6-pro and GPT-6 Pro as well.
