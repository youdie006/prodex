...
### Fixed
- Composer tools now recognize the measured `Add files and more` button when ChatGPT omits the old plus-button test ID. Lookup stays within one visible composer, rejects ambiguous/hidden/disabled controls, and strictly verifies the trigger's hit target before initial or retry clicks. A live no-send check activated Web search and restored the original empty composer. The [tools incident record](docs/composer-tools-incident-2026-09-26.md) also separates a reproduced stale-container transcript/captcha false positive from genuine human-verification requirements.
