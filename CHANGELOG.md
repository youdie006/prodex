...
- CI now schedules full release verification on Ubuntu 24.04 with Node 20/22/24, macOS 15 ARM and Intel with Node 22, and Windows Server 2025 with Node 22. Jobs install/check ripgrep, report their actual runtime, fail independently, and have a 40-minute deadline. This matrix is a verification gate, not a claim that every platform has already passed.

### Verification And Limits
- PASS: 173 browser/process/product-check tests on WSL, seven native Windows 502 regressions and typecheck; 178 storage and send tests on physical M3 after isolating independent native cases. The 502 checks use simulated page/CDP data, not a new live account request.
- Follow-up at `afec42d`: full WSL release verification passed (1,604 tests, 3 platform exclusions), as did physical M3 (1,603 tests, 4 exclusions), Ubuntu CI Node 20/22/24 and macOS ARM CI. A native Windows candidate installed in a path containing spaces passed actual `prodex.cmd` stdio MCP startup, tool discovery and explicit-cwd file reading. Windows CI exposed eight process-inspection diagnostic failures; the regression failed before correction, then 42 focused native Windows checks and 49 WSL checks passed, along with typecheck. macOS Intel passed 1,600 tests with three 30-second timeouts; splitting the independent cases and retaining a dedicated persistence deadline passed all five affected WSL cases. The complete matrix must be rerun before publication.
- PASS at compatibility commit `8569391`: WSL integrated suite (1,598 passed, 0 failed, 3 platform exclusions), typecheck/build, all three Ubuntu CI Node 20/22/24 release-verification jobs and macOS ARM CI. Physical M3 completed the full release-verification command, including installed-package MCP smoke and doctor. An earlier M3 verification command exported `PRODEX_NO_AUTO_LOGIN=1`, disabling five deliberately mocked recovery tests; rerunning without that conflicting harness override passed. macOS Intel exposed the timing-dependent contention fixture; Windows exposed a non-executable nested package bin. Candidate 0.40.18 passed native Windows metadata and actual tar-header inspection, plus WSL installed-package smoke. The final matrix rerun remains pending.
- The first integrated run exposed a regression from applying ordinary-file hard-link restrictions to internal lock records. That run was terminated without touching account processes; the policy was separated and the focused lock/storage checks passed afterward. Integrated WSL, physical M3 and native Windows reruns are in progress, and no new package has been published or installed yet.
- Interim PASS: 77 focused storage, configuration, follow-up and publishing-credential tests on WSL; 48 native Windows tests plus one explicitly POSIX-only exclusion; Windows and WSL real headless browser launch/evaluation/graceful-close/cleanup. M3's intermediate full suite had 1,567 passing, 5 failing and 2 skipped tests; all five persistence fault-injection fixtures passed after correction. The intermediate Windows suite improved to 1,463 passing, 107 failing and 4 skipped tests before the later fixture and path fixes. Final integrated matrix results are still pending; these intermediate runs are not a release approval.
- PASS: 101 POSIX storage/configuration/concurrency tests and typecheck; six native Windows regressions for replacement, concurrent creation, rollback and unchanged initialization. The earlier native storage run had 78 passing and 20 failing tests, exposing these bugs plus unsupported symlink fixtures and OS-specific mocks. These counts are intermediate evidence, not a completed all-platform release claim.
- Cross-platform work in progress: native Windows 11 x64 reproduced directory `fchmod` and `npm.cmd` launch failures. The initial full suite reported 1,049 passing, 503 failing and 3 skipped tests; many failures shared the directory permission cause, and others require platform-aware fixtures or symlink privileges. The dedicated native storage/junction regressions failed before correction and then passed (2 tests). POSIX storage/configuration regressions passed (94 tests). Native Windows uses a temporary Node 22.22.0 runtime, not WSL Node; no login window or account prompt was opened for these checks.
