  ...
  it("guides unknown subcommands with close-match suggestions", async () => {
    const cases = [
      ...
      },
      {
        ...
        expected:
          "Unknown pro browser subcommand: chek. Did you mean `prodex pro browser check`? Expected one of: login, ask, smoke, check, models, projects. Run `prodex pro browser --help`."
      },
      {
        args: ["tasks", "wat"],
...
`,
  ...
  it("pro browser rejects unknown helper subcommands with guidance", async () => {
    ...
    await expect(
      ...
      "Unknown pro browser subcommand: verify. Expected one of: login, ask, smoke, check, models, projects. Run `prodex pro browser --help`."
    );
  });
  ...
  it("prints browser-specific help from pro browser help", async () => {
    ...
    expect(text).toContain("prodex pro browser smoke [--source-cli /absolute/path/to/dist/cli.js] [--cwd /absolute/path/to/repo]");
    expect(text).toContain(
      'prodex pro browser ask [--source-cli /absolute/path/to/dist/cli.js] [--cwd /absolute/path/to/repo] [--port 9333] [--timeout-ms 90000] [--target-url url --confirm-target] [--new-chat] [--stdin] [--json] [--auto-login|--no-auto-login] [--file path] [--model Pro] [--pro-mode 기본|확장] [--effort 즉시|중간|높음|"매우 높음"] [--project "name" | --project-new "name"] "prompt"'
    );
  ...
  it("prints source-checkout browser-specific help when source-cli is supplied", async () => {
    ...
    expect(text).toContain(
      `${sourcePrefix} pro browser ask --source-cli ${sourceCli} [--cwd /absolute/path/to/repo] [--port 9333] [--timeout-ms 90000] [--target-url url --confirm-target] [--new-chat] [--stdin] [--json] [--auto-login|--no-auto-login] [--file path] [--model Pro] [--pro-mode 기본|확장] [--effort 즉시|중간|높음|"매우 높음"] [--project "name" | --project-new "name"] "prompt"`
    );
