...
`,
  ...
  it("prints browser-specific help from pro browser help", async () => {
    ...
    expect(text).toContain("--headed requests a visible interactive browser");
    expect(text).toContain("0/false/no values");
  });
  ...
  it("allows browser login handoff when DevTools is already reachable", async () => {
    ...
    expect(out.join("\n")).not.toContain("Opened the dedicated Chrome window");
    expect(out.join("\n")).toContain("1. The existing profile is open at https://chatgpt.com/c/review-thread; log in manually only if ChatGPT requests it.");
